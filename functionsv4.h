...
act initAct(act myact) {
    strcpy(myact.name,"");
    strcpy(myact.location,"");
    strcpy(myact.comment,"");
...
// 新增活动
int bill_insert(char *pathname, act *b,user uu)
{
    if(uu.status>=1)//管理员和根用户可以直接添加活动，无需审核
    {
...
int actauditing1(char *pathname, act *actlist)
{
    ...
    act tmp;
    int count = 0;
    while (fread(&tmp, sizeof(act), 1, fp))
    {
        if(tmp.auditing==0)
        {
...
int decision(char *pathname,sign_msg ss,user *uu)//返回1说明有相应的账号密码，-1说明无
{
    FILE *fp = fopen(pathname, "rb");
    user tmp;
    while (fread(&tmp, sizeof(act), 1, fp))
