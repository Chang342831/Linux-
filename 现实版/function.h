...
int actremove(int id)
{
    ...
    fclose(fp);
    fclose(fpn);
    remove(BRFILE);              // 删除原档案文件
    rename("./tmpfile", BRFILE); // 复制好的新文件重命名为档案文件
    return 1;
}

int actupdate(int id,int i)
{
    FILE *fp;
    struct act tmp;
    fp = fopen(BRFILE, "r+b");
